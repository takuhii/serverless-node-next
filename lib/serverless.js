...
// serverless-node-next: runtime auto-update disabled (see init()).
...
class Serverless {
  ...
  async init() {
    ...
    // serverless-node-next: runtime auto-update disabled.
    // This fork is installed/updated via npm/GitHub, so we do not perform
    // the background version check or self-update (previously:
    // `eventuallyUpdate(this)` from ./utils/eventually-update).
