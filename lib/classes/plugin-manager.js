...
class PluginManager {
  ...
  async loadAllPlugins(servicePlugins) {
    ...
    resolvedServicePlugins.filter(Boolean).forEach((Plugin) => this.addPlugin(Plugin));
    isRegisteringExternalPlugins = false;
  ...
  async requireServicePlugin(serviceDir, pluginPath, legacyLocalPluginsPath) {
    ...
    if (localPluginPath) {
      if (pluginPath.startsWith('./')) {
        this.localPluginsPaths.push({ resolvedPath: localPluginPath, inputPath: pluginPath });
      }
      return importModule(localPluginPath);
    }
    ...
    return importModule(externalPluginPath);
  }
  ...
  resolveEnterprisePlugin() {
    // serverless-node-next: the Serverless Dashboard plugin is not bundled.
    // Return a no-op stub so plugin loading and variable-source wiring keep
    // working without any dashboard/platform-client dependency.
    return require('../plugins/dashboard-stub');
  }
  ...
  async runHooks(hookName, hooks) {
    const debugLog = log.get('lifecycle:command:invoke:hook');
