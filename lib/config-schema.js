...
const schema = {
  ...
  properties: {
    ...
    // serverless-node-next: previously injected by @serverless/dashboard-plugin.
    // Defined here so existing configs using `org`/`app` still validate against
    // the closed base schema now that the dashboard plugin is removed.
    app: { type: 'string' },
    org: { type: 'string' },
