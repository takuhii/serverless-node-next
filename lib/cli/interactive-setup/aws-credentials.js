...
// serverless-node-next: dashboard is not supported; these are inert placeholders.
// All code paths that would use them are gated behind `isAuthenticated()`,
// which always returns false in this fork (see below), so they are never reached.
const dashboardFrontend = 'https://app.serverless.com';
...
// serverless-node-next: Serverless Dashboard is not supported in this fork.
// `isAuthenticated` is always false, so every dashboard-gated branch below is
// unreachable and the flow falls through to local AWS credential setup.
const isAuthenticated = () => false;
const getPlatformClientWithAccessKey = () => {
  throw new Error('Serverless Dashboard is not supported in serverless-node-next');
};
